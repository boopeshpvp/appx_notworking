...
module.exports = {
  ...
  deleteEducationalDetail: async (req, res) => {
    try {
      const details = await EmployeeDetail.findByIdAndUpdate(
        req.params.id_1,
        { $pull: { educationalData: { _id: req.params.id_2 } } },
        { new: true }
      );
      res.status(200).json({
        data: details,
  ...
  deleteProjectDetail: async (req, res) => {
    try {
      const details = await EmployeeDetail.findByIdAndUpdate(
        req.params.id_1,
        { $pull: { projectData: { _id: req.params.id_2 } } },
        { new: true }
      );
      res.status(200).json({
        data: details,
        message: "Projectdata deleted successfully",
      });
    } catch (error) {
      res.status(404).json({
        status: "Fail",
  ...
  deleteEmployeementDetail: async (req, res) => {
    try {
      const details = await EmployeeDetail.findByIdAndUpdate(
        req.params.id_1,
        { $pull: { employmentData: { _id: req.params.id_2 } } },
        { new: true }
      );
      res.status(200).json({
        data: details,
  ...
  editEducationalDetail: async (req, res) => {
    try {
      const employeeDetailId = req.params.id_1;
      const educationDetailId = req.params.id_2;
      const updatedFields = req.body;

      let updateObject = {};

      for (let key in updatedFields) {
        updateObject[`educationalData.$.${key}`] = updatedFields[key];
      }

      const details = await EmployeeDetail.findOneAndUpdate(
        { _id: employeeDetailId, "educationalData._id": educationDetailId },
        { $set: updateObject },
        { new: true }
      );

      res.status(200).json({
        data: details.educationalData,
  ...
  editEmployeementDetail: async (req, res) => {
    try {
      const employeeDetailId = req.params.id_1;
      const employeementDetailId = req.params.id_2;
      const updatedFields = req.body;
      let updatedObject = {};
      for (let key in updatedFields) {
        updatedObject[`employmentData.$.${key}`] = updatedFields[key];
      }
      const details = await EmployeeDetail.findOneAndUpdate(
        { _id: employeeDetailId, "employmentData._id": employeementDetailId },
        { $set: updatedObject },
        { new: true }
      );
      res.status(200).json({
        data: details.employmentData,
        message: "Employeement data updated successfully",
      });
    } catch (error) {
      res.status(404).json({
        status: "Fail",
  ...
  editProjectDetail: async (req, res) => {
    try {
      const employeeId = req.params.id_1;
      const projectDataId = req.params.id_2;
      const updatedFields = req.body;
      const updatedObject = {};
      for (let key in updatedFields) {
        updatedObject[`projectData.$.${key}`] = updatedFields[key];
      }

      const details = await EmployeeDetail.findOneAndUpdate(
        { _id: employeeId, "projectData._id": projectDataId },
        { $set: updatedObject },
        { new: true }
      );
      res.status(200).json({
        data: details.projectData,
  ...
  otpVerification: async (req, res) => {
    try {
      const detail = await EmployeeDetail.findOne({ otp: req.body.otp });
      if (!detail) {
        res.status(404).json({
          status: "Fail",
